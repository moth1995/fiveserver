...
def get_db() -> pymysql.connections.Connection:
    ...
    if 'db' not in g:
        ...
        host: str = os.environ.get('DB_HOST', db_cfg.get('readServers', ['127.0.0.1'])[0])
...
def find_user_by_id(conn: pymysql.connections.Connection,
...
def find_user_by_username(conn: pymysql.connections.Connection,
...
def find_user_by_hash(conn: pymysql.connections.Connection,
                      hash_val: str) -> dict[str, Any] | None:
    with conn.cursor() as cur:
        cur.execute(
...
def find_user_by_nonce(conn: pymysql.connections.Connection,
...
def browse_users(conn: pymysql.connections.Connection,
                 offset: int = 0,
                 limit: int = 50,
                 search: str | None = None) -> tuple[int, list[dict[str, Any]]]:
...
def create_user(conn: pymysql.connections.Connection,
                username: str,
                serial: str,
                hash_val: str) -> int:
...
def update_user(conn: pymysql.connections.Connection,
                user_id: int,
                username: str,
                serial: str,
                hash_val: str) -> None:
...
def lock_user(conn: pymysql.connections.Connection,
              user_id: int,
              nonce: str) -> None:
    with conn.cursor() as cur:
        cur.execute(
            'UPDATE users SET reset_nonce=%s WHERE id = %s',
            (nonce, user_id))
...
_PROFILE_COLS = ('id, user_id, ordinal, name, fav_player, fav_team, `rank`, '
                 'points, disconnects, updated_on, seconds_played')


def get_profiles_for_user(conn: pymysql.connections.Connection,
...
def get_profile_by_id(conn: pymysql.connections.Connection,
                      profile_id: int) -> dict[str, Any] | None:
    with conn.cursor() as cur:
        cur.execute(
...
def get_profile_by_name(conn: pymysql.connections.Connection,
...
def get_profile_stats(conn: pymysql.connections.Connection,
...
def create_profiles_for_user(conn: pymysql.connections.Connection,
                              user_id: int) -> None:
