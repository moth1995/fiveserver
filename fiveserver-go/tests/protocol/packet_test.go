...
func TestPacket_RoundTrip(t *testing.T) {
	p := protocol.Packet{
		Header: protocol.Header{ID: 0x2008, Length: 4, PacketCount: 1},
		Data:   []byte{0x01, 0x02, 0x03, 0x04},
	}
	got, err := protocol.Unmarshal(protocol.Marshal(p))
	if err != nil {
...
func TestPacket_EmptyData(t *testing.T) {
	p := protocol.Packet{
		Header: protocol.Header{ID: 0x0005, Length: 0, PacketCount: 99},
		Data:   []byte{},
	}
	got, err := protocol.Unmarshal(protocol.Marshal(p))
	if err != nil {
...
func TestPacket_MD5Corruption(t *testing.T) {
	p := protocol.Packet{
		Header: protocol.Header{ID: 0x3003, Length: 3, PacketCount: 5},
		Data:   []byte{0xAA, 0xBB, 0xCC},
	}
	wire := protocol.Marshal(p)
	wire[25] ^= 0xFF // flip a bit in the data
	if _, err := protocol.Unmarshal(wire); err == nil {
		t.Fatal("expected MD5 mismatch error, got nil")
	}
}

func TestPacket_HeaderBigEndian(t *testing.T) {
	h := protocol.Header{ID: 0x200A, Length: 0x0010, PacketCount: 0x00000003}
	b := protocol.MarshalHeader(h)
...
func TestPacket_MD5OverHeaderAndData(t *testing.T) {
	...
	d := []byte{0x01, 0x02}
	p1 := protocol.Packet{Header: protocol.Header{ID: 0x0001, Length: 2, PacketCount: 1}, Data: d}
	p2 := protocol.Packet{Header: protocol.Header{ID: 0x0002, Length: 2, PacketCount: 1}, Data: d}
	w1, w2 := protocol.Marshal(p1), protocol.Marshal(p2)
