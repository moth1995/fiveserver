...
func TestXorData_RoundTrip(t *testing.T) {
	...
	if got := XorData(XorData(input, 0), 0); !bytes.Equal(got, input) {
...
func TestXorData_KnownFixture_Start0(t *testing.T) {
	...
	if got := XorData(input, 0); !bytes.Equal(got, want) {
...
func TestXorData_OffsetWrap(t *testing.T) {
	...
	input := make([]byte, 4)
	if got0, got8 := XorData(input, 0), XorData(input, 8); !bytes.Equal(got0, got8) {
...
func TestXorData_Start1(t *testing.T) {
	...
	if got := XorData(input, 1); !bytes.Equal(got, want) {
